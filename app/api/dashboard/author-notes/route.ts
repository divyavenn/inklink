...
export async function POST(req: NextRequest) {
  ...
  const { chapterVersionId, body, selectedText, pollOptions } = await req.json();
  ...
  const wp = feedbackWordPos(ver.rendered_html as string, selectedText);
 ...
 * { id, body?, pollOptions? }
 *
 * Editing pollOptions when responses already exist clears the tallies (the
 * choice indices may no longer line up). The route resets responses for safety.
 */
export async function PATCH(req: NextRequest) {
  ...
  if (pollOptions !== undefined) {
    ...
    await sql`UPDATE author_notes SET poll_options = ${pollOptions ? JSON.stringify(pollOptions) : null} WHERE id = ${id}`;
  }
  if (body !== undefined) {
    if (typeof body === 'string' && body.length > 400) {
      return NextResponse.json({ error: 'Note body must be 400 characters or fewer' }, { status: 400 });
 ...
 * Deletes only the specified version of the note. Other versions of the same
...
export async function DELETE(req: NextRequest) {
  ...
  await sql`DELETE FROM author_notes WHERE id = ${id}`;
  return NextResponse.json({ ok: true });
}
